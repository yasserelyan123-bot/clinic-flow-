// ============================================================
// ClinicFlow — نسخة محدثة (Sidebar Layout + تحسينات)
// ============================================================
...
function showMessage(text, type = 'info') {
    ...
    window.__clinicflow_msg_timeout__ = setTimeout(() => { banner.style.display = 'none'; }, 5000);
}
...
class ClinicApp {
    ...
    get specialtyTheme() {
        ...
        const themes = {
            obgyn: { key: 'obgyn', color: 'pink', from: 'from-pink-500', to: 'to-rose-400', label: 'عيادة النساء والتوليد', icon: '🤰' },
            diabetes_hypertension: { key: 'diabetes_hypertension', color: 'amber', from: 'from-amber-500', to: 'to-orange-400', label: 'عيادة السكر والضغط', icon: '💊' },
            cardiology: { key: 'cardiology', color: 'rose', from: 'from-rose-600', to: 'to-red-400', label: 'عيادة القلب', icon: '❤️' },
            oncology: { key: 'oncology', color: 'purple', from: 'from-purple-600', to: 'to-fuchsia-400', label: 'عيادة الأورام', icon: '🎗️' },
            general: { key: 'general', color: 'blue', from: 'from-blue-600', to: 'to-cyan-400', label: 'عيادة عامة', icon: '🏥' },
        };
        return themes[specialty] || themes.general;
    }

    get commonConditionsForSpecialty() {
        const key = this.specialtyTheme.key;
        const lists = {
            diabetes_hypertension: ['السكري (Type 2 Diabetes)', 'السكري (Type 1 Diabetes)', 'ضغط الدم المرتفع (Hypertension)', 'الكوليسترول المرتفع'],
            cardiology: ['أمراض الشريان التاجي', 'قصور القلب', 'عدم انتظام ضربات القلب', 'ارتفاع ضغط الدم الرئوي'],
            oncology: ['سرطان الثدي', 'سرطان القولون', 'سرطان الرئة', 'سرطان الدم (اللوكيميا)', 'سرطان أخرى'],
            general: COMMON_CONDITIONS,
            obgyn: COMMON_CONDITIONS,
        };
        return lists[key] || COMMON_CONDITIONS;
    }
    ...
    navigate(page, data = null) {
        ...
        if (page === 'invoices') this.loadInvoices();
    }
    ...
    async loadDashboard() {
        ...
        try {
            ...
            const el1 = document.getElementById('today-appointments');
            const el2 = document.getElementById('total-patients');
            const el3 = document.getElementById('total-visits');
            const el4 = document.getElementById('active-pregnancies');
            if (el1) el1.textContent = todayCount;
            if (el2) el2.textContent = patients.length;
            if (el3) el3.textContent = visits.length;
            if (el4) el4.textContent = pregnancies.length;
        ...
        try {
            const allConditions = await pgFetch('chronic_conditions?status=eq.active&select=*,patients(name)&order=created_at.desc&limit=40', { accessToken: token });
            const isCancer = (name) => /سرطان|cancer|ورم خبيث|أورام/i.test(name || '');
            const isCardiac = (name) => /قلب|شريان تاجي|cardiac|heart/i.test(name || '');
            const oncologyOnly = (allConditions || []).filter(c => isCancer(c.condition_name)).slice(0, 8);
            const cardiologyOnly = (allConditions || []).filter(c => !isCancer(c.condition_name) && isCardiac(c.condition_name)).slice(0, 8);
            const chronicOnly = (allConditions || []).filter(c => !isCancer(c.condition_name) && !isCardiac(c.condition_name)).slice(0, 8);
            ...
            if (chronicEl) {
                chronicEl.innerHTML = chronicOnly.length
                    ...
                    : '<p class="text-gray-400 text-center py-6">لا توجد حالات نشطة</p>';
            }
            const cardiologyEl = document.getElementById('cardiology-conditions-list');
            if (cardiologyEl) {
                cardiologyEl.innerHTML = cardiologyOnly.length
                    ? cardiologyOnly.map(c => renderConditionRow(c, 'rose')).join('')
                    : '<p class="text-gray-400 text-center py-6">لا توجد حالات قلب نشطة</p>';
            }
    ...
    async loadPatientDetail() {
            ...
            if (this._currentPatient) {
                ...
                document.getElementById('patient-detail-header').innerHTML = `
                    <div class="flex justify-between items-start">
                        ...
                        <button id="goto-invoice-btn" class="text-xs bg-green-600 text-white px-3 py-1.5 rounded-lg hover:bg-green-700">💵 إنشاء فاتورة</button>
                    </div>`;
                const gotoInvoiceBtn = document.getElementById('goto-invoice-btn');
                if (gotoInvoiceBtn) gotoInvoiceBtn.addEventListener('click', () => this.navigate('invoices'));
            }
        ...
        this.loadPatientFiles(id, token);
    }

    // ---------- Patient Files (أشعة + تحاليل) ----------
    async loadPatientFiles(id, token) {
        const el = document.getElementById('patient-files-section');
        if (!el) return;
        try {
            const files = await pgFetch(`patient_files?patient_id=eq.${id}&select=*&order=created_at.desc`, { accessToken: token });
            const typeLabels = { xray: 'أشعة', lab: 'تحاليل', ultrasound: 'سونار', other: 'أخرى' };
            const typeIcons = { xray: '🦴', lab: '🧪', ultrasound: '📡', other: '📎' };

            el.innerHTML = `
                <button id="upload-file-btn" class="w-full bg-indigo-600 text-white py-2 rounded-lg text-sm hover:bg-indigo-700 mb-3">+ رفع أشعة / تحاليل / ملف</button>
                <div id="upload-file-form-container"></div>
                <div id="files-list">
                    ${(files && files.length) ? files.map(f => `
                        <div class="border border-gray-100 rounded-lg p-3 mb-2 flex justify-between items-center gap-2">
                            <div class="overflow-hidden">
                                <p class="font-semibold text-sm truncate">${typeIcons[f.file_type] || '📎'} ${esc(f.file_name)}</p>
                                <p class="text-xs text-gray-500">${typeLabels[f.file_type] || f.file_type} · ${fmtDate(f.created_at)}</p>
                                ${f.notes ? `<p class="text-xs text-gray-400 mt-1">${esc(f.notes)}</p>` : ''}
                            </div>
                            <a href="${esc(f.file_url)}" target="_blank" rel="noopener" class="text-xs bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-lg hover:bg-indigo-100 whitespace-nowrap">عرض</a>
                        </div>
                    `).join('') : '<p class="text-gray-400 text-sm text-center py-4">لا توجد ملفات مرفوعة بعد</p>'}
                </div>
            `;
            document.getElementById('upload-file-btn').addEventListener('click', () => this.showUploadFileForm());
        } catch (e) {
            el.innerHTML = `<p class="text-red-500 text-sm">تعذّر تحميل الملفات: ${esc(e.message)}</p>`;
        }
    }

    showUploadFileForm() {
        const container = document.getElementById('upload-file-form-container');
        if (!container) return;
        container.innerHTML = `
            <form id="upload-file-form" class="space-y-2 mb-4 bg-gray-50 p-3 rounded-lg border border-indigo-100">
                <select name="file_type" required class="w-full px-3 py-2 border rounded-lg text-sm">
                    <option value="">نوع الملف</option>
                    <option value="xray">🦴 أشعة</option>
                    <option value="lab">🧪 تحاليل</option>
                    <option value="ultrasound">📡 سونار / ألتراساوند</option>
                    <option value="other">📎 أخرى</option>
                </select>
                <input type="file" name="file" accept="image/*,.pdf,.doc,.docx" required class="w-full text-sm">
                <input name="notes" placeholder="ملاحظات (اختياري)" class="w-full px-3 py-2 border rounded-lg text-sm">
                <button type="submit" class="w-full bg-indigo-600 text-white py-2 rounded-lg text-sm hover:bg-indigo-700">رفع الملف</button>
            </form>
        `;
        document.getElementById('upload-file-form').addEventListener('submit', (e) => this.uploadPatientFile(e));
    }

    async uploadPatientFile(event) {
        event.preventDefault();
        const form = new FormData(event.target);
        ...
        if (!file || !file.size) {
            showMessage('اختر ملفًا أولاً', 'error');
            return;
        }
        if (file.size > 10 * 1024 * 1024) {
            showMessage('حجم الملف كبير جدًا (الحد الأقصى 10 ميجا)', 'error');
            return;
        }
        ...
        const path = `${clinicId}/${patientId}/${Date.now()}_${Math.random().toString(36).slice(2, 8)}.${ext}`;

        try {
            showMessage('جاري رفع الملف...', 'info');

            // رفع الملف إلى Storage
            const uploadRes = await fetch(`${API_BASE}/storage/v1/object/patient-files/${path}`, {
                method: 'POST',
                headers: {
            ...
            if (!uploadRes.ok) {
                ...
                throw new Error('فشل رفع الملف: ' + (errText || uploadRes.status));
            }

            const publicUrl = `${API_BASE}/storage/v1/object/public/patient-files/${path}`;

            // حفظ البيانات في الجدول
            await pgFetch('patient_files', {
                method: 'POST',
                body: {
                    clinic_id: clinicId,
            ...
            this.loadPatientFiles(patientId, this.token);
        } catch (e) {
            showMessage('خطأ في الرفع: ' + e.message, 'error');
        }
    }
    ...
    async loadPatientVisits(id, token) {
        ...
        try {
            ...
            container.innerHTML = `
                ...
                <div id="visits-history">
                    ${(visits && visits.length) ? visits.map(v => `
                        <div class="border-b border-gray-100 py-3">
                            ...
                            ${v.chief_complaint ? `<p class="text-xs text-gray-500 mt-1">الشكوى: ${esc(v.chief_complaint)}</p>` : ''}
                            ${v.bp_systolic ? `<p class="text-xs text-gray-500">ضغط: ${v.bp_systolic}/${v.bp_diastolic} ${v.weight ? '· الوزن: ' + v.weight + ' كجم' : ''}</p>` : ''}
                            ${v.plan ? `<p class="text-xs text-gray-500 mt-1">الخطة العلاجية: ${esc(v.plan)}</p>` : ''}
                            ${(v.prescriptions && v.prescriptions.length) ? `
                                <div class="mt-2 bg-blue-50 rounded-lg p-3">
                                    <div class="flex justify-between items-center mb-2">
                                        <p class="text-xs font-semibold text-blue-700">الوصفة الطبية:</p>
                                        <button data-vid="${v.id}" class="print-prescription-btn text-xs bg-blue-600 text-white px-2 py-1 rounded hover:bg-blue-700">🖨️ طباعة الروشتة</button>
                                    </div>
                                    ${v.prescriptions.map(p => `<p class="text-xs text-gray-700 mb-1">• <strong>${esc(p.drug_name)}</strong> ${p.dose ? '- ' + esc(p.dose) : ''} ${p.frequency ? '- ' + esc(p.frequency) : ''} ${p.duration_days ? '- ' + p.duration_days + ' يوم' : ''}</p>`).join('')}
                                </div>` : ''}
                            <button data-vid="${v.id}" class="add-prescription-btn text-xs text-blue-600 hover:underline mt-2">+ إضافة دواء للوصفة</button>
                            <div class="prescription-form-slot" data-slot="${v.id}"></div>
                        </div>`).join('') : '<p class="text-gray-400 text-sm text-center py-4">لا توجد زيارات مسجلة</p>'}
                </div>`;
            document.getElementById('new-visit-btn').addEventListener('click', () => this.showNewVisitForm());
            container.querySelectorAll('.add-prescription-btn').forEach(btn => {
                btn.addEventListener('click', () => this.showPrescriptionForm(btn.dataset.vid));
            });
            container.querySelectorAll('.print-prescription-btn').forEach(btn => {
                btn.addEventListener('click', () => this.printPrescription(btn.dataset.vid));
            });
        } catch (e) { showMessage('تعذّر تحميل الزيارات: ' + e.message, 'error'); }
    }

    showNewVisitForm() {
        const container = document.getElementById('new-visit-form-container');
        container.innerHTML = `
            <form id="new-visit-form" class="space-y-2 mb-4 bg-gray-50 p-3 rounded-lg">
                <textarea name="chief_complaint" placeholder="شكوى المريض (سبب الزيارة)" required class="w-full px-3 py-2 border rounded-lg text-sm"></textarea>
                <textarea name="examination" placeholder="الفحص السريري (اختياري)" class="w-full px-3 py-2 border rounded-lg text-sm"></textarea>
                <input name="diagnosis" placeholder="التشخيص" class="w-full px-3 py-2 border rounded-lg text-sm">
                <textarea name="plan" placeholder="الخطة العلاجية" class="w-full px-3 py-2 border rounded-lg text-sm"></textarea>
                ...
                <div class="grid grid-cols-2 gap-2">
                    ...
                    <input name="temperature" type="number" step="0.1" placeholder="الحرارة (°C)" class="px-3 py-2 border rounded-lg text-sm">
                </div>
                <input name="followup_date" type="date" placeholder="موعد المتابعة القادم" class="w-full px-3 py-2 border rounded-lg text-sm">
                <button type="submit" class="w-full bg-blue-600 text-white py-2 rounded-lg text-sm hover:bg-blue-700">حفظ الزيارة</button>
            </form>`;
        document.getElementById('new-visit-form').addEventListener('submit', (e) => this.createVisit(e));
    }
    ...
    async createVisit(event) {
        ...
        try {
            const payload = {
                ...
                examination: form.get('examination') || null,
                ...
                followup_date: form.get('followup_date') || null,
            };
            await pgFetch('visits', { method: 'POST', body: payload, accessToken: this.token });
            showMessage('تم تسجيل الزيارة بنجاح', 'success');
            this.loadPatientVisits(this.selectedPatient.id, this.token);
        } catch (e) { showMessage('خطأ: ' + e.message, 'error'); }
    }

    showPrescriptionForm(visitId) {
        const slot = document.querySelector(`.prescription-form-slot[data-slot="${visitId}"]`);
        if (!slot) return;
        slot.innerHTML = `
            <form class="prescription-form space-y-2 mt-2 bg-white border border-blue-200 rounded-lg p-3 shadow-sm">
                <p class="text-xs font-semibold text-blue-700 mb-1">إضافة دواء للروشتة</p>
                <input name="drug_name" placeholder="اسم الدواء *" required class="w-full px-3 py-2 border rounded-lg text-sm">
                <div class="grid grid-cols-2 gap-2">
                    <input name="dose" placeholder="الجرعة (مثال: 500 مجم)" class="px-3 py-2 border rounded-lg text-sm">
                    <input name="frequency" placeholder="التكرار (مثال: 3 مرات يوميًا)" class="px-3 py-2 border rounded-lg text-sm">
                </div>
                <div class="grid grid-cols-2 gap-2">
                    <input name="duration_days" type="number" placeholder="المدة بالأيام" class="px-3 py-2 border rounded-lg text-sm">
                    <input name="instructions" placeholder="تعليمات إضافية" class="px-3 py-2 border rounded-lg text-sm">
                </div>
                <button type="submit" class="w-full bg-blue-600 text-white py-2 rounded-lg text-sm hover:bg-blue-700 font-medium">+ إضافة الدواء</button>
            </form>`;
        slot.querySelector('.prescription-form').addEventListener('submit', (e) => this.addPrescriptionItem(e, visitId));
    }

    async addPrescriptionItem(event, visitId) {
        event.preventDefault();
        const form = new FormData(event.target);
        try {
            const payload = {
                ...
                dose: form.get('dose') || null,
                frequency: form.get('frequency') || null,
                duration_days: form.get('duration_days') ? parseInt(form.get('duration_days')) : null,
                instructions: form.get('instructions') || null,
            };
            await pgFetch('prescriptions', { method: 'POST', body: payload, accessToken: this.token });
            showMessage('تمت إضافة الدواء للوصفة', 'success');
            this.loadPatientVisits(this.selectedPatient.id, this.token);
        } catch (e) { showMessage('خطأ: ' + e.message, 'error'); }
    }

    async printPrescription(visitId) {
        try {
            const visits = await pgFetch(`visits?id=eq.${visitId}&select=*,prescriptions(*),patients(name,patient_number,gender,date_of_birth,phone)`, { accessToken: this.token });
            const visit = visits && visits[0];
            if (!visit || !visit.prescriptions || !visit.prescriptions.length) {
                showMessage('لا توجد أدوية في هذه الروشتة', 'error');
                return;
            }

            const patient = visit.patients || this._currentPatient || {};
            const clinicName = this.clinic?.name || 'ClinicFlow';
            const doctorName = this.profile?.full_name || 'الطبيب';
            const theme = this.specialtyTheme;

            const drugsHtml = visit.prescriptions.map((p, i) => `
                <tr>
                    <td style="padding:8px;border-bottom:1px solid #eee;text-align:center;">${i + 1}</td>
                    <td style="padding:8px;border-bottom:1px solid #eee;font-weight:600;">${esc(p.drug_name)}</td>
                    <td style="padding:8px;border-bottom:1px solid #eee;">${esc(p.dose || '-')}</td>
                    <td style="padding:8px;border-bottom:1px solid #eee;">${esc(p.frequency || '-')}</td>
                    <td style="padding:8px;border-bottom:1px solid #eee;">${p.duration_days ? p.duration_days + ' يوم' : '-'}</td>
                </tr>
            `).join('');
            ...
            printWindow.document.write(`
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>روشتة طبية - ${esc(patient.name || '')}</title>
                    <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700&display=swap" rel="stylesheet">
                    <style>
                        * { margin: 0; padding: 0; box-sizing: border-box; }
                        body { font-family: 'Cairo', sans-serif; padding: 30px; color: #222; direction: rtl; }
                        ...
                        .clinic-info h1 { font-size: 22px; color: #1e40af; }
                        .clinic-info p { font-size: 13px; color: #666; margin-top: 4px; }
                        .rx-title { text-align: center; font-size: 20px; font-weight: 700; color: #1e40af; margin: 15px 0; letter-spacing: 2px; }
                        .patient-box { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 16px; margin-bottom: 20px; display: grid; grid-template-columns: 1fr 1fr; gap: 8px; font-size: 14px; }
                        .patient-box strong { color: #475569; }
                        table { width: 100%; border-collapse: collapse; margin-bottom: 25px; }
                        th { background: #1e40af; color: white; padding: 10px 8px; font-size: 13px; text-align: right; }
                        td { font-size: 14px; }
                        .footer { margin-top: 40px; display: flex; justify-content: space-between; align-items: flex-end; }
                        .signature { text-align: center; }
                        .signature .line { width: 180px; border-bottom: 1px solid #333; margin: 40px auto 8px; }
                        .notes { font-size: 12px; color: #666; margin-top: 20px; border-top: 1px dashed #ccc; padding-top: 10px; }
                        @media print {
                            body { padding: 15px; }
                ...
                <body>
                    <div class="header">
                        <div class="clinic-info">
                            ...
                            <p>${theme.label}</p>
                            <p>د. ${esc(doctorName)}</p>
                        </div>
                        <div style="text-align:left;font-size:13px;color:#666;">
                            <p>تاريخ الروشتة: ${fmtDate(visit.created_at)}</p>
                            <p>رقم الملف: ${esc(patient.patient_number || '-')}</p>
                        </div>
                    </div>

                    <div class="rx-title">℞ روشتة طبية</div>

                    <div class="patient-box">
                        <div><strong>اسم المريض:</strong> ${esc(patient.name || '-')}</div>
                        <div><strong>النوع:</strong> ${patient.gender === 'female' ? 'أنثى' : 'ذكر'}</div>
                        <div><strong>الهاتف:</strong> ${esc(patient.phone || '-')}</div>
                        <div><strong>التشخيص:</strong> ${esc(visit.diagnosis || visit.chief_complaint || '-')}</div>
                    </div>
                    ...
                    ${visit.plan ? `<div class="notes"><strong>تعليمات عامة:</strong> ${esc(visit.plan)}</div>` : ''}

                    <div class="footer">
                        <div style="font-size:12px;color:#888;">
                            تم الإنشاء بواسطة ClinicFlow<br>
                            ${new Date().toLocaleString('ar-EG')}
                        </div>
                        <div class="signature">
                            <div class="line"></div>
                            <p style="font-size:13px;">توقيع الطبيب</p>
                            <p style="font-size:12px;color:#555;">د. ${esc(doctorName)}</p>
                        </div>
                    </div>

                    <div class="no-print" style="margin-top:30px;text-align:center;">
                        <button onclick="window.print()" style="background:#2563eb;color:white;border:none;padding:12px 30px;border-radius:8px;font-size:16px;cursor:pointer;font-family:Cairo;">
                            🖨️ طباعة الروشتة
                        </button>
                        <button onclick="window.close()" style="background:#e5e7eb;color:#333;border:none;padding:12px 30px;border-radius:8px;font-size:16px;cursor:pointer;margin-right:10px;font-family:Cairo;">
                            إغلاق
                        </button>
                    </div>
                </body>
            ...
            showMessage('تعذّر تحضير الروشتة للطباعة: ' + e.message, 'error');
        }
    }
    ...
    async loadPatientChronic(id, token) {
        ...
        try {
            ...
            const optionsHtml = this.commonConditionsForSpecialty.map(c => `<option value="${esc(c)}">`).join('');
            el.innerHTML = `
    ...
    async loadInvoices() {
        try {
            const data = await pgFetch('invoices?select=*,patients(name,patient_number)&order=created_at.desc', { accessToken: this.token });
            const container = document.getElementById('invoices-list');
            if (!container) return;
            const statusLabels = { draft: 'مسودة', issued: 'صادرة', paid: 'مدفوعة', partially_paid: 'مدفوعة جزئيًا', cancelled: 'ملغاة' };
            const statusColors = { draft: 'gray', issued: 'blue', paid: 'green', partially_paid: 'amber', cancelled: 'red' };
            container.innerHTML = (data && data.length)
                ? data.map(inv => `
                    <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-3">
                        <div class="flex justify-between items-center">
                            <div>
                                <h3 class="font-bold text-gray-800">${esc(inv.patients?.name || 'مريض')}</h3>
                                <p class="text-xs text-gray-500">رقم الملف: ${esc(inv.patients?.patient_number || '-')} · ${fmtDate(inv.created_at)}</p>
                            </div>
                            <div class="text-left">
                                <p class="font-bold text-gray-800">${inv.grand_total} ج.م</p>
                                <span class="text-xs bg-${statusColors[inv.status] || 'gray'}-50 text-${statusColors[inv.status] || 'gray'}-700 px-2 py-1 rounded-full">${statusLabels[inv.status] || inv.status}</span>
                            </div>
                        </div>
                        ${inv.status !== 'paid' && inv.status !== 'cancelled' ? `<button data-iid="${inv.id}" class="mark-paid-btn text-xs text-green-600 hover:underline mt-2">✓ تحديد كمدفوعة</button>` : ''}
                    </div>`).join('')
                : '<p class="text-gray-400 text-center py-10">لا توجد فواتير بعد</p>';
            container.querySelectorAll('.mark-paid-btn').forEach(btn => {
                btn.addEventListener('click', () => this.markInvoicePaid(btn.dataset.iid));
            });
        } catch (e) { showMessage('تعذّر تحميل الفواتير: ' + e.message, 'error'); }

        try {
            const patients = await pgFetch('patients?select=id,name,patient_number', { accessToken: this.token });
            this._patientsCache = patients || [];
            const list = document.getElementById('invoice-patient-list');
            if (list) list.innerHTML = this._patientsCache.map(p => `<option value="${esc(p.name)} (${esc(p.patient_number)})">`).join('');
        } catch (e) { console.error(e); }
    }
    ...
    async createInvoice(event) {
        ...
        const match = this._patientsCache.find(p => `${p.name} (${p.patient_number})` === patientLabel);
        if (!match) { showMessage('اختر مريضًا من القائمة المقترحة', 'error'); return; }
        const total = parseFloat(form.get('total_amount')) || 0;
        const discount = parseFloat(form.get('discount')) || 0;
        const tax = parseFloat(form.get('tax')) || 0;
        ...
        try {
            ...
            await pgFetch('invoices', { method: 'POST', body: payload, accessToken: this.token });
    ...
    async markInvoicePaid(invoiceId) {
        try {
            await pgFetch(`invoices?id=eq.${invoiceId}`, { method: 'PATCH', body: { status: 'paid' }, accessToken: this.token });
            showMessage('تم تحديد الفاتورة كمدفوعة', 'success');
            this.loadInvoices();
        } catch (e) { showMessage('خطأ: ' + e.message, 'error'); }
    }

    // ---------- Render shell (Sidebar Layout) ----------
    render() {
        const app = document.getElementById('app');
        if (!this.profile) {
            document.body.className = 'bg-gradient-to-br from-slate-50 to-blue-50 min-h-screen';
        ...
        app.innerHTML = `
            <div class="flex h-screen overflow-hidden relative">
                ...
                <!-- Sidebar -->
                <aside id="sidebar" class="fixed md:static inset-y-0 right-0 w-64 bg-white border-l border-gray-200 flex flex-col shadow-lg z-40
                    transform translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out">
                    <div class="p-5 border-b border-gray-100 flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-xl bg-gradient-to-br ${theme.from} ${theme.to} flex items-center justify-center text-white text-xl shadow-md">
                                ${theme.icon}
                            </div>
                            <div class="overflow-hidden">
                                <h1 class="font-bold text-gray-800 text-sm truncate">${esc(this.clinic?.name || 'ClinicFlow')}</h1>
                                <p class="text-xs text-gray-500 truncate">${theme.label}</p>
                            </div>
                        </div>
                        <button id="close-sidebar-btn" class="md:hidden p-2 text-gray-500 hover:text-gray-800 text-xl">✕</button>
                    </div>

                    <nav class="flex-1 p-3 space-y-1 overflow-y-auto">
                        <button data-nav="dashboard" class="nav-btn w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all
                            ${this.currentPage === 'dashboard' ? `bg-gradient-to-l ${theme.from} ${theme.to} text-white shadow-md` : 'text-gray-600 hover:bg-gray-100'}">
                            <span class="text-lg">📊</span>
                            <span>الرئيسية</span>
                        </button>
                        <button data-nav="patients" class="nav-btn w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all
                            ${['patients', 'patient-detail'].includes(this.currentPage) ? `bg-gradient-to-l ${theme.from} ${theme.to} text-white shadow-md` : 'text-gray-600 hover:bg-gray-100'}">
                            <span class="text-lg">👥</span>
                            <span>المرضى</span>
                        </button>
                        <button data-nav="appointments" class="nav-btn w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all
                            ${this.currentPage === 'appointments' ? `bg-gradient-to-l ${theme.from} ${theme.to} text-white shadow-md` : 'text-gray-600 hover:bg-gray-100'}">
                            <span class="text-lg">📅</span>
                            <span>المواعيد</span>
                        </button>
                        <button data-nav="invoices" class="nav-btn w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all
                            ${this.currentPage === 'invoices' ? `bg-gradient-to-l ${theme.from} ${theme.to} text-white shadow-md` : 'text-gray-600 hover:bg-gray-100'}">
                            <span class="text-lg">💰</span>
                            <span>الفواتير</span>
                        </button>
                    </nav>
                    ...
                    <div class="p-4 border-t border-gray-100">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="w-9 h-9 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 font-bold text-sm">
                                ${(this.profile.full_name || 'U').charAt(0)}
                            </div>
                            <div class="overflow-hidden">
                                <p class="text-sm font-medium text-gray-800 truncate">${esc(this.profile.full_name || '')}</p>
                                <p class="text-xs text-gray-500">${this.profile.role === 'admin' ? 'مدير' : this.profile.role}</p>
                            </div>
                        </div>
                        <button id="logout-btn" class="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm text-red-600 hover:bg-red-50 transition-colors border border-red-100">
                            <span>🚪</span>
                ...
                <div class="flex-1 flex flex-col overflow-hidden w-full">
                    <header class="bg-white border-b border-gray-200 px-4 md:px-6 py-3 md:py-4 flex items-center justify-between shadow-sm">
                        <div class="flex items-center gap-3">
                            <button id="open-sidebar-btn" class="md:hidden p-2 rounded-lg bg-gray-100 text-gray-700 text-xl">☰</button>
                            <div>
                                <h2 class="text-base md:text-lg font-bold text-gray-800">
                                    ${this.currentPage === 'dashboard' ? 'لوحة التحكم' :
                                      this.currentPage === 'patients' || this.currentPage === 'patient-detail' ? 'إدارة المرضى' :
                                      this.currentPage === 'appointments' ? 'المواعيد' :
                                      this.currentPage === 'invoices' ? 'الفواتير' : ''}
                                </h2>
                                <p class="text-xs text-gray-500 mt-0.5 hidden sm:block">${theme.label}</p>
                            </div>
                        </div>
        ...
        app.querySelectorAll('.nav-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                closeSidebar(); // تقفل القائمة بعد الاختيار على الموبايل
            });
        });
    ...
    renderAuthPages() {
                ...
                <div class="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
                    ...
                    <form id="complete-signup-form" class="space-y-3">
                        ...
                        <select name="specialty" required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg">
                            ...
                            <option value="diabetes_hypertension">💊 السكر والضغط</option>
                            <option value="cardiology">❤️ أمراض القلب</option>
                            <option value="oncology">🎗️ الأورام</option>
        ...
        if (this.currentPage === 'forgot-password') {
            ...
            <div class="min-h-screen flex items-center justify-center bg-gray-50 p-4">
                <div class="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
                    <h1 class="text-2xl font-bold text-blue-600 mb-2">🔐 استعادة كلمة المرور</h1>
                    <p class="text-sm text-gray-500 mb-4">أدخل بريدك الإلكتروني وسنرسل لك رابط إعادة تعيين كلمة المرور</p>
                    <form id="forgot-password-form" class="space-y-3">
                        ...
                        <button type="submit" class="w-full bg-blue-600 text-white py-2.5 rounded-lg hover:bg-blue-700">إرسال رابط الاستعادة</button>
                    </form>
                    <p class="mt-4 text-sm text-center"><a href="#" id="go-login" class="text-blue-600">العودة لتسجيل الدخول</a></p>
                </div>
            </div>`;
        }
            ...
            <div class="min-h-screen flex items-center justify-center bg-gray-50 p-4">
                <div class="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
                    ...
                    <form id="register-form" class="space-y-3">
                        ...
                        <input name="email" type="email" placeholder="البريد الإلكتروني" required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg">
                        <div class="relative">
                            <input name="password" id="reg-password" type="password" placeholder="كلمة المرور (6 أحرف على الأقل)" required minlength="6" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg pl-12">
                            <button type="button" class="toggle-password absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-lg" data-target="reg-password">👁️</button>
                        </div>
                        <button type="submit" class="w-full bg-blue-600 text-white py-2.5 rounded-lg hover:bg-blue-700">إنشاء العيادة</button>
                    </form>
        ...
        <div class="min-h-screen flex items-center justify-center p-4">
            <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                <div class="bg-gradient-to-l from-blue-600 to-cyan-500 -mx-8 -mt-8 mb-6 px-8 py-8 text-center">
                    <h1 class="text-3xl font-bold text-white">🏥 ClinicFlow</h1>
                    <p class="text-white/80 text-sm mt-1">نظام إدارة العيادات المتخصصة</p>
                </div>
                <form id="login-form" class="space-y-3">
                    <input name="email" type="email" placeholder="البريد الإلكتروني" required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg">
                    <div class="relative">
                        <input name="password" id="login-password" type="password" placeholder="كلمة المرور" required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg pl-12">
                        <button type="button" class="toggle-password absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-lg" data-target="login-password">👁️</button>
                    </div>
                    <button type="submit" class="w-full bg-gradient-to-l from-blue-600 to-cyan-500 text-white py-2.5 rounded-lg font-semibold hover:opacity-90">دخول</button>
                </form>
                <div class="mt-4 flex justify-between text-sm">
                    <a href="#" id="go-forgot" class="text-gray-500 hover:text-blue-600">نسيت كلمة المرور؟</a>
                    <a href="#" id="go-register" class="text-blue-600 font-medium">عيادة جديدة؟ أنشئ حسابًا</a>
                </div>
            </div>
        </div>`;
    }
    ...
    bindAuthForms() {
        ...
        if (forgotForm) forgotForm.addEventListener('submit', (e) => this.forgotPassword(e));
        ...
        if (goForgot) goForgot.addEventListener('click', (e) => { e.preventDefault(); this.currentPage = 'forgot-password'; this.render(); });
    ...
    async forgotPassword(event) {
        ...
        try {
            const res = await fetch(`${API_BASE}/auth/v1/recover`, {
                method: 'POST',
                headers: {
                    'apikey': ANON_KEY,
                    'Content-Type': 'application/json',
            ...
            const data = await res.json().catch(() => ({}));
            if (!res.ok) {
                throw new Error(data.error_description || data.msg || data.error || 'فشل إرسال رابط الاستعادة');
            }
            showMessage('تم إرسال رابط إعادة تعيين كلمة المرور إلى بريدك الإلكتروني. تحقق من صندوق الوارد (والبريد المزعج).', 'success');
            this.currentPage = 'login';
            this.render();
        } catch (e) {
    ...
    renderCurrentPage() {
        switch (this.currentPage) {
            ...
            case 'invoices': return this.renderInvoicesPage();
    ...
    renderDashboard() {
        ...
        const showDiabetes = specialty === 'diabetes_hypertension' || specialty === 'general';
        const showCardiology = specialty === 'cardiology' || specialty === 'general';
        const showOncology = specialty === 'oncology' || specialty === 'general';
        const theme = this.specialtyTheme;

        const statCards = [
            `<div class="bg-white p-5 rounded-xl shadow-md border-t-4 border-${theme.color}-400">
            ...
            `<div class="bg-white p-5 rounded-xl shadow-md border-t-4 border-green-400">
                <h3 class="text-gray-400 text-xs mb-1">إجمالي المرضى</h3>
                <p id="total-patients" class="text-2xl font-bold text-green-600">0</p>
            </div>`,
            `<div class="bg-white p-5 rounded-xl shadow-md border-t-4 border-violet-400">
                <h3 class="text-gray-400 text-xs mb-1">إجمالي الزيارات</h3>
                <p id="total-visits" class="text-2xl font-bold text-violet-600">0</p>
            </div>`,
        ];
        if (showPregnancy) {
            statCards.push(`<div class="bg-white p-5 rounded-xl shadow-md border-t-4 border-pink-400">
        ...
        const sections = [];
        if (showPregnancy) {
            sections.push(`<div class="bg-white p-5 rounded-xl shadow-md">
        ...
        if (showDiabetes) {
            sections.push(`<div class="bg-white p-5 rounded-xl shadow-md">
                <h2 class="font-bold text-gray-800 mb-3">💊 متابعة السكر والضغط</h2>
        ...
        if (showCardiology) {
            sections.push(`<div class="bg-white p-5 rounded-xl shadow-md">
                <h2 class="font-bold text-gray-800 mb-3">❤️ متابعة حالات القلب</h2>
                <div id="cardiology-conditions-list"></div>
            </div>`);
        } else {
            sections.push('<div id="cardiology-conditions-list" style="display:none"></div>');
        }
        if (showOncology) {
            sections.push(`<div class="bg-white p-5 rounded-xl shadow-md">
    ...
    renderInvoicesPage() {
        return `
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-1">
                    <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                        <h2 class="font-bold text-gray-800 mb-4">إنشاء فاتورة جديدة</h2>
                        <form id="invoice-form" class="space-y-3">
                            <input name="patient_search" list="invoice-patient-list" placeholder="ابحث عن اسم المريض" required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg">
                            <datalist id="invoice-patient-list"></datalist>
                            <input name="total_amount" type="number" step="0.01" placeholder="قيمة الخدمة (ج.م)" required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg">
                            <div class="grid grid-cols-2 gap-2">
                                <input name="discount" type="number" step="0.01" placeholder="الخصم" class="px-4 py-2.5 border border-gray-200 rounded-lg">
                                <input name="tax" type="number" step="0.01" placeholder="الضريبة" class="px-4 py-2.5 border border-gray-200 rounded-lg">
                            </div>
                            <button type="submit" class="w-full bg-green-600 text-white py-2.5 rounded-lg hover:bg-green-700">إنشاء الفاتورة</button>
                        </form>
                    </div>
                </div>
                <div class="lg:col-span-2">
                    <h2 class="font-bold text-gray-800 mb-4">الفواتير</h2>
    ...
    renderPatientDetailPage() {
        ...
        const showChronic = clinical && specialty !== 'obgyn';
        const chronicMeta = {
            oncology: { title: 'متابعة الأورام والقياسات', icon: '🎗️' },
            cardiology: { title: 'متابعة القلب والقياسات', icon: '❤️' },
            diabetes_hypertension: { title: 'متابعة السكر والضغط', icon: '💊' },
            general: { title: 'الأمراض المزمنة والقياسات', icon: '💊' },
        }[specialty] || { title: 'الأمراض المزمنة والقياسات', icon: '💊' };
        ...
        if (showChronic) {
            columns.push(`
            <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                <h3 class="font-bold text-gray-800 mb-3">${chronicMeta.icon} ${chronicMeta.title}</h3>
        ...
        // قسم الملفات (أشعة + تحاليل) يظهر دائمًا
        columns.push(`
            <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                <h3 class="font-bold text-gray-800 mb-3">📁 الأشعة والتحاليل</h3>
                <div id="patient-files-section"></div>
            </div>
        `);
        ...
        return `
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">${columns.join('')}</div>
        `;
    }
}
