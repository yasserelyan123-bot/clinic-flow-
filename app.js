...
class ClinicApp {
    ...
    // خرائط التخصص: كل تخصص له هوية بصرية مستقلة (لون متدرج + عنوان + أيقونة)
    get specialtyTheme() {
        let specialty = this.clinic?.specialty || 'general';
        if (specialty === 'chronic') specialty = 'diabetes_hypertension'; // توافق مع حسابات قديمة
        const themes = {
    ...
    // قائمة الأمراض المقترحة تختلف حسب تخصص العيادة، عشان تحس إنه برنامج مخصص لتخصصك
    ...
    render() {
        ...
        document.body.className = `bg-gradient-to-b from-${theme.color}-50 to-white min-h-screen`;
        app.innerHTML = `
            <nav class="bg-gradient-to-l ${theme.from} ${theme.to} shadow-lg sticky top-0 z-40">
                <div class="max-w-5xl mx-auto px-4">
                    <div class="flex justify-between h-20 items-center">
                        <div>
                            <h1 class="text-xl font-bold text-white flex items-center gap-2">${theme.icon} ${esc(this.clinic?.name || 'ClinicFlow')}</h1>
                            <p class="text-xs text-white/80 mt-0.5">${theme.label}</p>
                        </div>
                        <div class="flex items-center gap-1 overflow-x-auto">
                            <button data-nav="dashboard" class="nav-btn px-3 py-2 rounded-lg text-sm font-medium ${this.currentPage === 'dashboard' ? 'bg-white text-gray-800 shadow' : 'text-white hover:bg-white/20'}">الرئيسية</button>
                            <button data-nav="patients" class="nav-btn px-3 py-2 rounded-lg text-sm font-medium ${['patients', 'patient-detail'].includes(this.currentPage) ? 'bg-white text-gray-800 shadow' : 'text-white hover:bg-white/20'}">المرضى</button>
                            <button data-nav="appointments" class="nav-btn px-3 py-2 rounded-lg text-sm font-medium ${this.currentPage === 'appointments' ? 'bg-white text-gray-800 shadow' : 'text-white hover:bg-white/20'}">المواعيد</button>
                            <button data-nav="invoices" class="nav-btn px-3 py-2 rounded-lg text-sm font-medium ${this.currentPage === 'invoices' ? 'bg-white text-gray-800 shadow' : 'text-white hover:bg-white/20'}">الفواتير</button>
                            <button id="logout-btn" class="px-3 py-2 rounded-lg text-sm font-medium text-white/90 hover:bg-white/20">خروج</button>
                        </div>
                    </div>
                </div>
            </nav>
