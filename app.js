...
class ClinicApp {
    ...
    async loadProfile() {
        try {
            ...
            if (!this.profile) {
                console.warn('لا يوجد صف في profiles لهذا المستخدم:', this.session.user.id);
            }
        } catch (e) {
            console.error(e);
            this.profile = null;
        ...
        if (this.currentPage === 'complete-signup') {
            ...
            <div class="min-h-screen flex items-center justify-center bg-gray-100">
                <div class="bg-white p-8 rounded-lg shadow w-full max-w-md">
                    ...
                    <p class="text-sm text-gray-600 mb-4">حسابك موجود لكن لم يتم العثور على ملف العيادة. أكمل البيانات:</p>
                    ${this._lastProfileError ? `<div class="bg-red-50 border border-red-300 text-red-700 text-sm p-3 rounded-lg mb-4">تفاصيل تقنية: ${this._lastProfileError}</div>` : ''}
                    ...
                    <button id="complete-signup-logout" class="w-full mt-4 text-red-600 text-sm hover:underline">
