...
class ClinicApp {
    ...
    async finishClinicRegistration() {
        try {
            await pgRpc('register_clinic', { p_clinic_name: this._pendingClinicName, p_admin_full_name: this._pendingAdminName }, this.token);
            await this.loadProfile();
    ...
    async loadDashboard() {
        ...
        try {
            const conditions = await pgFetch('chronic_conditions?status=eq.active&select=*,patients(name)&order=created_at.desc&limit=8', { accessToken: token });
            const el = document.getElementById('chronic-conditions-list');
            if (el) {
                el.innerHTML = (conditions && conditions.length)
                    ? conditions.map(c => `
                        <div class="flex justify-between items-center border-b border-gray-100 py-3">
                            <div>
                            ...
                            <span class="text-xs bg-amber-50 text-amber-700 px-2 py-1 rounded-full">${esc(c.status)}</span>
                        </div>`).join('')
            ...
            <nav class="bg-white shadow-sm sticky top-0 z-40">
                <div class="max-w-5xl mx-auto px-4">
                    <div class="flex justify-between h-16 items-center">
                        <h1 class="text-xl font-bold text-blue-600 flex items-center gap-2">🏥 ClinicFlow</h1>
                        <div class="flex items-center gap-1 overflow-x-auto">
                            <button data-nav="dashboard" class="nav-btn px-3 py-2 rounded-lg text-sm ${this.currentPage === 'dashboard' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100'}">الرئيسية</button>
                            <button data-nav="patients" class="nav-btn px-3 py-2 rounded-lg text-sm ${['patients', 'patient-detail'].includes(this.currentPage) ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100'}">المرضى</button>
                            <button data-nav="appointments" class="nav-btn px-3 py-2 rounded-lg text-sm ${this.currentPage === 'appointments' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100'}">المواعيد</button>
        ...
        return `
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                    ...
                    <p id="today-appointments" class="text-2xl font-bold text-blue-600">0</p>
                </div>
                <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                    <h2 class="font-bold text-gray-800 mb-3">حالات مزمنة نشطة (سكري، ضغط، سرطان...)</h2>
    ...
    renderPatientDetailPage() {
        return `
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                    <h3 class="font-bold text-gray-800 mb-3">الأمراض المزمنة والقياسات</h3>
