...
class ClinicApp {
                        ...
                        <div class="border-b border-gray-100 py-3">
                            ...
                            ${(v.prescriptions && v.prescriptions.length) ? `
                                <div class="mt-2 bg-blue-50 rounded-lg p-2">
                                    <p class="text-xs font-semibold text-blue-700 mb-1">الوصفة الطبية:</p>
                                    ${v.prescriptions.map(p => `<p class="text-xs text-gray-700">• ${esc(p.drug_name)} ${p.dose ? '- ' + esc(p.dose) : ''} ${p.frequency ? '- ' + esc(p.frequency) : ''} ${p.duration_days ? '- ' + p.duration_days + ' يوم' : ''}</p>`).join('')}
                                </div>` : ''}
    ...
    showPrescriptionForm(visitId) {
        ...
        if (!slot) return;
        slot.innerHTML = `
            <form class="prescription-form space-y-1 mt-2 bg-white border rounded-lg p-2">
                <input name="drug_name" placeholder="اسم الدواء" required class="w-full px-2 py-1.5 border rounded text-xs">
                <div class="grid grid-cols-2 gap-1">
                    <input name="dose" placeholder="الجرعة" class="px-2 py-1.5 border rounded text-xs">
                    <input name="frequency" placeholder="عدد المرات يوميًا" class="px-2 py-1.5 border rounded text-xs">
                </div>
                <input name="duration_days" type="number" placeholder="المدة بالأيام" class="w-full px-2 py-1.5 border rounded text-xs">
                <button type="submit" class="w-full bg-blue-600 text-white py-1.5 rounded text-xs hover:bg-blue-700">إضافة للوصفة</button>
            </form>`;
